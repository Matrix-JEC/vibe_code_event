import React from 'react'
import contact_img from '../assets/contact_img2.png'
import { TiSocialLinkedin } from "react-icons/ti";
import { RiFacebookFill } from "react-icons/ri";
import { FaInstagram } from "react-icons/fa";
import { FaYoutube } from "react-icons/fa";

export default function Contact() {
  return (
    <div className='bg-black p-16 lg:px-60 h-screen min-h-fit text-white w-[100vw] min-w-fit '>
        <p className='text-[#FF0105] font-[800] text-6xl lg:p-4 pb-4 mb-4'>How Can We Help You ?</p>
        <div className='bg-[#1A1A1A] relative lg:pl-16 pt-8 flex justify-between rounded-2xl p-4 min-w-fit min-h-fit lg:w-full w-[100vw]'>
            <div className=''>
            <p className='w-80 font-[600] text-5xl text-white'>
                We’re here to connect and assist you
            </p>
            <br />
            <p className='py-4'>
                Have questions, need assistance, or just want to connect?
                <br/>
                Our team is ready to assist you.
            </p>
            <div className='grid grid-cols-2 gap-12 w-full mt-10'>
                <div>
                    <div className='text-[#FF0105] font-[600] text-2xl'>CONTACT US</div>
                    <br/>
                    <div>+916267xxxxxx</div>
                </div>
                <div>
                    <div className='text-[#FF0105] font-[600] text-2xl'>
                        EVENT LOCATION
                    </div>
                    <br/>
                    <div>Jabalpur Engineering College </div>
                </div>
                <div>
                    <div className='text-[#FF0105] font-[600] text-2xl'>EMAIL</div>
                    <br/>
                    <div>team.matrix.jec@gmail.com</div>
                </div>
                <div>
                    <div className='text-[#FF0105] font-[600] text-2xl'>FOLLOW US</div>
                    <br/>
                    <div className='ml-4 flex gap-4 '>
                        <RiFacebookFill className='cursor-pointer bg-white rounded-full w-5 h-5 text-[#1A1A1A]'/>
                        
                        <TiSocialLinkedin className='cursor-pointer bg-white w-5 h-5 rounded p-0.5 text-[#1A1A1A]' />
                        <FaInstagram className='cursor-pointer w-5 h-5 rounded text-white'  />
                        <FaYoutube className='cursor-pointer bg-[#1A1A1A] w-5 h-5 rounded text-white' />
                    </div>
                    </div>
                </div>

            </div>
            
            <div className='hidden md:block w-1/2 h-full relative min-w-fit'>
                <img src={contact_img} alt="" className='w-full h-full object-cover'/>
                
            </div>
           
        </div>
      
    </div>
  )
}
