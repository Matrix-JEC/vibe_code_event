  ...
  return (
    <>
      <div className='relative w-fit block ml:hidden text-white'
        ...
        {display && <nav className='absolute flex-col min-w-fit flex py-2 rounded-lg mt-0.5 z-101 w-48 -left-12 overflow-hidden bg-[#000000c4]' style={{ backdropFilter: "blur(3px)" }}>
          {navList.map((ele) => {
      ...
      {!!display && <div className='w-dvw h-dvh absolute z-100 top-0 bg-transparent -right-[34dvw]' onClick={(e) => { setDisplay((display) => !display) }}></div>}
    </>
  )
}